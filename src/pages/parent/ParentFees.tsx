...
export function ParentFees() {
    ...
    const handleDownloadReceipt = () => {
        ...
        // Simulate download using print for better user experience
        setTimeout(() => {
            window.print();
        }, 500);
    };
                                        ...
                                        <div key={item.id} className="p-4 hover:bg-muted/30 transition-colors">
                                            <div className="flex justify-between items-start mb-3">
                                                <div className="min-w-0 flex-1">
                                                    <p className="font-semibold text-sm mb-1 truncate">{item.students?.full_name || item.students?.name || 'N/A'}</p>
                                                    <p className="text-sm text-muted-foreground mb-2">{feeType}</p>
                                                </div>
                                                <Badge
                                    ...
                                    feeRecords.map((item: any) => {
                                        ...
                                        return (
                                            <tr key={item.id} className="border-b border-border hover:bg-muted/50 transition-colors">
                                                <td className="py-4 px-4 text-sm font-medium">{item.students?.full_name || item.students?.name || 'N/A'}</td>
                                                <td className="py-4 px-4 text-sm max-w-[200px] truncate">{feeType}</td>
                                                <td className="py-4 px-4 text-sm font-semibold">₹ {item.amount_due.toLocaleString()}</td>
                                                ...
                                                <td className="py-4 px-4">
                                                    {item.status === 'paid' ? (
                                                        ...
                                                        <Button
                                                            ...
                                                            disabled={isPaying === item.id}
                                                            onClick={() => handlePayNow(item)}
                                                            className="bg-primary text-white min-h-[36px]"
                                                        >
                                                            {isPaying === item.id ? (
                                                                <div className="flex items-center gap-2">
                                                                    <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
