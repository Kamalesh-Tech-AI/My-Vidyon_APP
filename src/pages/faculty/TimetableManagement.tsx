...
export function TimetableManagement() {
    ...
    // NO MERGING of special slots into classTimetableData here.
    // We will handle them separately in the render logic to keep views distinct.
    ...
    // Calculate days to show (Single row for special mode)
    let daysToShow = DAYS.slice(0, configData.days_per_week);
    if (isSpecialMode) {
        const date = new Date(specialClassDate);
    ...
    // Merge special slots for the CURRENT FACULTY into myScheduleData
    // We only show special slots for the selected specialClassDate in the weekly grid
    // to avoid confusion (since weekly grid is for recurring periods)
    if (specialClassDate) {
        const selectedDate = new Date(specialClassDate);
        const dayOfWeek = selectedDate.getDay(); // 0 is Sunday
        const dayIndex = dayOfWeek === 0 ? 6 : dayOfWeek - 1; // Adjust to 0=Mon, 5=Sat
        const selectedDayName = DAYS[dayIndex];

        specialSlots.forEach((slot: any) => {
            if (slot.faculty_id === user?.id) {
                // Try to map back to a period based on start_time
                // This is a heuristic: match exact start_time or find closest
                let matchedPeriod = -1;
    ...
    const saveSpecialSlotMutation = useMutation({
        mutationFn: async () => {
            ...
            const slotData = {
                ...
                title: specialClassTitle, // Include reason
            };
    ...
    // Batch send notifications mutation for Special Classes
    const sendBatchNotificationMutation = useMutation({
        mutationFn: async () => {
            if (!user?.id || !user?.institutionId || !specialClassDate) {
                throw new Error('Missing required data');
            }

            // 1. Fetch special slots for this faculty and date
            ...
            if (!slots || slots.length === 0) {
                throw new Error('No special slots found for today to notify about.');
            }
            ...
            slots.forEach(s => {
                if (s.class_id) uniqueCombos.add(`${s.class_id}|${s.section}`);
            });

            // Fetch class data to get names if needed
            const { data: allClassData } = await supabase.from('classes').select('id, name');

            for (const combo of uniqueCombos) {
                const [cid, sec] = combo.split('|');
                const className = allClassData?.find(c => String(c.id) === String(cid))?.name;

                if (!className) continue;

                const { data: students } = await supabase
                    .from('students')
                    .select('id, email, parent_id') // Fetch email
                    .eq('class_name', className)
                if (students && students.length > 0) {
                    // Fetch profiles for these students to get Auth IDs
                    ...
                    students.forEach(s => {
                        // Use profile ID if exists (registered user), else fallback to student ID
                        // Normalize student email to lowercase before lookup
                        const studentEmail = s.email?.toLowerCase();
                        const targetUserId = (studentEmail && profileMap.get(studentEmail)) || s.id;

                        notifications.push({
                            user_id: targetUserId,
                            title: specialClassTitle || 'Special Timetable Added',
                            message: `A special timetable has been added to your schedule for ${specialClassDate} by ${user.name}.${specialClassTitle ? ` Reason: ${specialClassTitle}` : ''}`,
                            type: 'timetable',
                        ...
                        if (s.parent_id) {
                            notifications.push({
                                user_id: s.parent_id,
                                title: `Special Timetable: ${specialClassTitle || 'Update'}`,
        ...
        onSuccess: () => {
            toast.success('Notifications sent to all assigned students and parents');
        },
        onError: (err) => {
    ...
    const isLoading = isLoadingSchedule || isLoadingClassTimetable || isLoadingAssignment || isLoadingSpecial;
    ...
    const canManageClass = isClassTeacher;
                                                                ...
                                                                {slot?.subject_id ? (
                                                                    <div className={`space-y-1 p-1 rounded-md ${slot.is_special ? 'bg-orange-50 ring-1 ring-orange-200' : ''}`}>
                                                                        ...
                                                                        {slot.is_special && slot.title && (
                                                                            <div className="text-[9px] text-orange-700 font-bold px-1 truncate italic">
                                                                                {slot.title}
                                                                            </div>
                                                                        )}
                            ...
                            <Card className="overflow-hidden">
                                <CardContent className="flex flex-col items-center justify-center py-12 text-center text-muted-foreground bg-white">
                                    ...
                                    <p className="max-w-sm mt-2 text-sm">
                                        Only Class Teachers can manage the weekly timetable for their assigned class and section.
                                    </p>
                                </CardContent>
                            </Card>
                        ) : (
                                        ...
                                        <div className="p-4 bg-orange-50 border-b flex flex-col gap-4">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <Button
                                                    ...
                                                    size="sm"
                                                    className="h-8 text-xs bg-indigo-600 hover:bg-indigo-700 text-white gap-2"
                                                >
                                                    <Users className="w-3.5 h-3.5" />
                                                    {sendBatchNotificationMutation.isPending ? "Sending..." : "Notify Students"}
                                                </Button>
                                            </div>

                                            <div className="flex items-end gap-4">
                                                <div className="space-y-1.5">
                                                    <label className="text-xs font-semibold text-orange-800">Event Date</label>
                                                    <div className="flex items-center gap-2 px-3 h-9 bg-white rounded-md border border-orange-200 shadow-sm w-[160px]">
                                                        <Calendar className="w-3.5 h-3.5 text-orange-400" />
                                                        <Input
                                                            type="date"
                                                            className="h-full w-full border-none p-0 text-sm focus-visible:ring-0"
                                                ...
                                                <div className="space-y-1.5 flex-1">
                                                    <label className="text-xs font-semibold text-orange-800">Reason / Title</label>
                                                    <Input
                                                        placeholder="e.g. Extra Class for Exams"
                                                        className="h-9 bg-white border-orange-200 focus-visible:ring-orange-200"
                                                        ...
                                                        {Array.from({ length: configData.periods_per_day }, (_, i) => i + 1).map((period) => {
                                                            ...
                                                            if (isSpecialMode) {
                                                                ...
                                                                slot = specialSlots.find((s: any) => {
                                                                    ...
                                                                    // Strict matching for assignments
